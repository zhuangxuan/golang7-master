...
func Marshal(v interface{}) ([]byte, error) {
	...
	case reflect.Map:
		bf.WriteByte('{')
		if value.Len() > 0 { //如果map里面存在值
			//方法1
			for _, reflectmap_key := range value.MapKeys() { //用反射遍历map，取出所有key
				reflectmap_value := value.MapIndex(reflectmap_key) //通过key得到value
				key := reflectmap_key.Interface()                  //将反射得来的key转成原始类型
				bf.WriteString("\"")
				bf.WriteString(fmt.Sprintf("%v", key))
				bf.WriteString(":")
				if bs, err := Marshal(reflectmap_value.Interface()); err != nil { //如果存在嵌套，则对value递归调用Marshal序列化
					return nil, err
				} else {
					bf.Write(bs)
				}
			...
			//方法2
			// iter := value.MapRange() //用反射遍历map，iter为这个map的K、V对
			// for iter.Next() {//
			// 	k := iter.Key()
			// 	v := iter.Value()
			// 	bf.WriteString("\"")
			...
			// 	bf.WriteString("\"")
			// 	bf.WriteString(":")
			// 	if bs, err := Marshal(v.Interface()); err != nil { //如果存在嵌套，则对value递归调用Marshal序列化
			// 		return nil, err
			// 	} else {
			// 		bf.Write(bs)
			// 	}
...
func Unmarshal(data []byte, v interface{}) error {
	...
	case reflect.Map:
		if s[0] == '{' && s[len(s)-1] == '}' {
			arr := SplitJson(s[1 : len(s)-1])
			if len(arr) > 0 {
				value.Set(reflect.MakeMapWithSize(typ, len(arr))) //通过反射创建一个map
				mapkeyType := typ.Key()                           //通过反射获取map里面的key的反射类型
				mapkeyValue := reflect.New(mapkeyType)            //通过反射获取map里面的key的反射值
				mapvalue_Type := typ.Elem()                       //通过反射获取map里面的value的反射类型
				mapvalue_Value := reflect.New(mapvalue_Type)      //通过反射获取map里面的value的反射值

				for _, ele := range arr {
					brr := strings.SplitN(ele, ":", 2) //json的value里可能存在嵌套，所以用:分隔时限定个数为2
					if len(brr) == 2 {                 //如果是一组k,v对
						//去掉两边的空格
						mapkey := strings.Trim(brr[0], " ")
						mapvalue := strings.Trim(brr[1], " ")
						if mapkey[0] != '"' && mapkey[len(mapkey)-1] != '"' {
							return fmt.Errorf("错误:%s格式有误", mapkey)
						}
						//去掉两边的"符号
						mapkey = strings.Trim(mapkey, "\"")
						mapvalue = strings.Trim(mapvalue, "\"")

						if err := Unmarshal([]byte(mapkey), mapkeyValue.Interface()); err != nil { //递归调用Unmarshal，给map的key赋值
							return err
						}
						if err := Unmarshal([]byte(mapvalue), mapvalue_Value.Interface()); err != nil { //递归调用Unmarshal，给map的value赋值
							return err
						}
						value.SetMapIndex(mapkeyValue.Elem(), mapvalue_Value.Elem()) //最后将k,v对给map赋值，得到的map
					} else {
						return fmt.Errorf("错误:%s格式有误", ele)
					}
				}
