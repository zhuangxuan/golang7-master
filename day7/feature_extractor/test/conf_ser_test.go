...
import (
	"encoding/json"
	"fmt"
	"go-course/feature_extractor/common"
	"go-course/feature_extractor/extractor"
	"log"
	"os"
	"testing"
	"time"
...
func init() {
	...
	if err := fe.Init("/Users/zhangchaoyang/go_project/go-course/feature_extractor/conf/feature_extractor.json"); err != nil {
		log.Fatal(err)
	}
}
...
func TestSerializeConf(t *testing.T) {
	...
	fout, err := os.OpenFile("/Users/zhangchaoyang/go_project/go-course/feature_extractor/conf/test.json", os.O_CREATE|os.O_TRUNC|os.O_WRONLY, 0666)
	if err != nil {
		fmt.Println(err)
		t.Fail()
	} else {
